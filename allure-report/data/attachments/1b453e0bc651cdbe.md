...
- Name: tests\groceryapp.spec.js >> grocery shopping application >> @smoke Manage user
...
Call log:
  - waiting for locator('tr').filter({ hasText: 'Hari' }).first().locator('a[href*="/admin/User/status"]')
...
```ts
  ...
  46 | 
> 47 |                         await row.locator('a[href*="/admin/User/status"]').click();
     |                                                                            ^ Error: locator.click: Test timeout of 40000ms exceeded.
  48 | 
  49 |                 }
  50 | }
